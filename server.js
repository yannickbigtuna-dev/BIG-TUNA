...
const LAMP_DIR          = path.join(DATA, 'lamp');
...
const LAMP_STATE_FILE = path.join(LAMP_DIR, 'state.json');
const LAMP_DEVICE_STATUS_FILE = path.join(LAMP_DIR, 'device-status.json');
const LAMP_NATIVE_COMMANDS_FILE = path.join(LAMP_DIR, 'native-commands.json');
...
// prompt independently keeps only its newest 18 hints.
...
for (const dir of [DATA, CLIMBS_DIR, SETTINGS_DIR, APPDATA_DIR, MEETS_DIR, CLIMBV2_DIR, QUIZZES_DIR, SHARED_LISTS_DIR, LIGHTS_DIR, LAMP_DIR, RADAR_DIR, ASSIGNMENTS_DIR, ANALYTICS_EVENTS_DIR, EMAIL_TEMPLATES_DIR, EMAIL_CAMPAIGNS_DIR])
...
function getLampDeviceStatusPayload() {
  ...
  return {
    ...
    receivedAt: status.trustedReceivedAt,
    polledAt: status.trustedPolledAt,
    recentlyPolled: lastPoll > 0 && Date.now() - lastPoll <= LAMP_DEVICE_RECENT_MS,
...
function readNativeLampSessions() {
  try { const value = JSON.parse(fs.readFileSync(LAMP_NATIVE_SESSIONS_FILE, 'utf8')); return Array.isArray(value) ? value : []; }
  catch { return []; }
}

function writeNativeLampSessions(sessions) {
  const now = Date.now();
  atomicWrite(LAMP_NATIVE_SESSIONS_FILE, sessions.filter(session => Date.parse(session.expiresAt) > now).slice(-32));
}

function getNativeLampUser(token) {
  const websiteUser = getSessionUser(token);
  if (websiteUser) return websiteUser;
  const session = readNativeLampSessions().find(item => safeTokenEqual(item.token, token) && Date.parse(item.expiresAt) > Date.now());
  return session ? readUsers().find(user => user.id === session.userId) || null : null;
}
...
function sendLampSse(res, state) {
  const { on, updatedAt } = state;
  res.write(`data: ${JSON.stringify({ on, updatedAt })}\n\n`);
}

function broadcastLampState(state) {
  if (!lampSseClients.size) return;
  for (const client of [...lampSseClients]) {
    try { sendLampSse(client.res, state); } catch {}
  }
}
...
// only the signed-in account's connection state, never its tokens, email, or
...
async function getChallengeAccountStravaStatus(user) {
  ...
  if (!participantId) return { connected: false, lastSyncAt: null, athlete: null };
  try {
    const status = await stravaChallenge.getAdminStatus();
    const participant = Array.isArray(status && status.participants)
      ? status.participants.find(value => value && value.id === participantId)
      : null;
    return {
      connected: Boolean(participant && participant.connected),
      lastSyncAt: participant && typeof participant.lastSyncAt === 'string' ? participant.lastSyncAt : null,
      athlete: participant && participant.stravaAthleteId ? { id: String(participant.stravaAthleteId) } : null,
    };
  } catch {
    return { connected: false, lastSyncAt: null, athlete: null };
  }
}
...
function challengeAccountsUser(req, res) {
  const user = getSessionUser(getToken(req));
  if (!user) { jsonRes(res, 401, { error: 'Not authenticated' }); return null; }
  if (!allowChallengeApiRequest(user, req)) { jsonRes(res, 429, { error: 'Too many challenge requests. Please retry shortly.' }); return null; }
  return user;
}
...
async function challengeAccountsBody(req, res) {
  try { return await parseBoundedJson(req, 64 * 1024); }
  catch (error) {
    jsonRes(res, error && error.code === 'BODY_TOO_LARGE' ? 413 : 400, { error: error && error.code === 'BODY_TOO_LARGE' ? 'Request body is too large' : 'Request body must be valid JSON' });
    return null;
  }
}
...
function challengeCreateInput(user, body) {
  ...
  const yannick = accounts.find(account => String(account.username || '').trim().toLowerCase() === 'yannick');
  const emma = accounts.find(account => ['fishyemma', 'emma'].includes(String(account.username || '').trim().toLowerCase()));
  const participants = [
    yannick && { userId: yannick.id, role: yannick.id === user.id ? 'owner' : 'admin' },
    emma && { userId: emma.id, role: emma.id === user.id ? 'owner' : 'member' },
    { userId: user.id, role: 'owner' },
  ].filter(Boolean).filter((participant, index, all) => all.findIndex(value => value.userId === participant.userId) === index);
  ...
  try {
    ...
    const activities = Array.isArray(dashboard && dashboard.currentWeek && dashboard.currentWeek.activities)
      ? dashboard.currentWeek.activities.map(activity => ({
        id: `strava_${String(activity.id || '')}`,
        userId: ownerByLegacyParticipant[activity.participantId],
...
async function handleAPI(req, res, urlPath) {
  ...
  if (urlPath === '/api/challenge-accounts/me' && req.method === 'GET') {
    setSensitiveResponseHeaders(res);
    const user = challengeAccountsUser(req, res);
    if (!user) return;
    if (!challengeAccounts) return jsonRes(res, 503, { error: 'Challenge accounts are temporarily unavailable' });
    try { return jsonRes(res, 200, await challengeAccounts.getProfile(user)); }
    catch (error) { return challengeAccountsError(res, error); }
  }
  ...
  if (urlPath === '/api/challenges' && ['GET', 'POST'].includes(req.method)) {
    ...
    if (!challengeAccounts) return jsonRes(res, 503, { error: 'Challenge accounts are temporarily unavailable' });
    if (req.method === 'GET') {
      try { return jsonRes(res, 200, { challenges: await challengeAccounts.listChallenges(user) }); }
      catch (error) { return challengeAccountsError(res, error); }
    }
    const body = await challengeAccountsBody(req, res);
    if (body === null) return;
    try { return jsonRes(res, 201, await challengeAccounts.createChallenge(user, challengeCreateInput(user, body))); }
    catch (error) { return challengeAccountsError(res, error); }
  }
  ...
  if (urlPath === '/api/challenge-devices' && req.method === 'POST') {
    ...
    const body = await challengeAccountsBody(req, res);
    if (body === null) return;
    try { return jsonRes(res, 201, await challengeAccounts.registerDevice(user, body)); }
    catch (error) { return challengeAccountsError(res, error); }
  }
  ...
  if (challengeReviewsMatch && ['GET', 'POST'].includes(req.method)) {
    setSensitiveResponseHeaders(res);
    const user = challengeAccountsUser(req, res);
    if (!user) return;
    if (!challengeAccounts) return jsonRes(res, 503, { error: 'Challenge accounts are temporarily unavailable' });
    if (req.method === 'GET') {
      ...
      try { return jsonRes(res, 200, { reviews: await challengeAccounts.listReviewRequests(user, challengeReviewsMatch[1], { status: status || undefined }) }); }
      catch (error) { return challengeAccountsError(res, error); }
    }
    const body = await challengeAccountsBody(req, res);
    if (body === null) return;
  ...
  const challengeReviewDecisionMatch = urlPath.match(/^\/api\/challenges\/([A-Za-z0-9_-]{1,64})\/review-requests\/([A-Za-z0-9_-]{1,96})\/decision$/);
  if (challengeReviewDecisionMatch && req.method === 'POST') {
    setSensitiveResponseHeaders(res);
    const user = challengeAccountsUser(req, res);
    if (!user) return;
    if (!challengeAccounts) return jsonRes(res, 503, { error: 'Challenge accounts are temporarily unavailable' });
    const body = await challengeAccountsBody(req, res);
    if (body === null) return;
    try { return jsonRes(res, 200, await challengeAccounts.decideReviewRequest(user, challengeReviewDecisionMatch[1], challengeReviewDecisionMatch[2], body)); }
    catch (error) { return challengeAccountsError(res, error); }
  }

  const challengeNotificationEventsMatch = urlPath.match(/^\/api\/challenges\/([A-Za-z0-9_-]{1,64})\/notification-events$/);
  if (challengeNotificationEventsMatch && req.method === 'GET') {
    setSensitiveResponseHeaders(res);
    const user = challengeAccountsUser(req, res);
    if (!user) return;
    if (!challengeAccounts) return jsonRes(res, 503, { error: 'Challenge accounts are temporarily unavailable' });
  ...
  // POST /api/lights - public legacy web desired-state control
  if (req.method === 'POST' && urlPath === '/api/lights') {
    try {
      const body = await parseBoundedJson(req);
      if (!body || typeof body !== 'object' || Array.isArray(body)
          || Object.keys(body).length !== 1 || typeof body.on !== 'boolean') {
        return jsonRes(res, 400, { error: 'Body must contain only on' });
      }

      const { on, updatedAt } = writeLightsState(body.on, 'public');
  ...
  if (req.method === 'POST' && urlPath === '/api/lamp/native/v1/session') {
    ...
    if (String(user.username || '').toLowerCase() !== 'yannick') return jsonRes(res, 403, { error: 'Forbidden' });
    const token = generateToken();
    const sessions = readNativeLampSessions();
    sessions.push({ token, userId: user.id, createdAt: new Date().toISOString(), expiresAt: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString() });
    writeNativeLampSessions(sessions);
    return jsonRes(res, 200, { token, username: user.username });
  }

  if (req.method === 'DELETE' && urlPath === '/api/lamp/native/v1/session') {
    const token = getToken(req);
    if (token) writeNativeLampSessions(readNativeLampSessions().filter(session => !safeTokenEqual(session.token, token)));
    return jsonRes(res, 200, { ok: true });
  }

  if ((req.method === 'GET' || req.method === 'PUT') && urlPath === '/api/lamp/native/v1') {
    const user = getNativeLampUser(getToken(req));
    if (!user) return jsonRes(res, 401, { error: 'Not authenticated' });
    if (String(user.username || '').toLowerCase() !== 'yannick') {
      return jsonRes(res, 403, { error: 'Forbidden' });
    }
  ...
  // GET /api/lamp/events - public live desired lamp state stream
  if (req.method === 'GET' && urlPath === '/api/lamp/events') {
    res.writeHead(200, {
      'Content-Type': 'text/event-stream',
      'Cache-Control': 'no-cache, no-store, must-revalidate',
      'Connection': 'keep-alive',
    ...
    const client = { res };
    lampSseClients.add(client);
    sendLampSse(res, readLampState());
    ...
    req.on('close', () => {
      clearInterval(ping);
      lampSseClients.delete(client);
    });
    return;
  }

  // GET /api/lamp - public desired lamp state
  if (req.method === 'GET' && urlPath === '/api/lamp') {
  ...
  // GET /api/lamp/homekit - pairing status stub
  if (req.method === 'GET' && urlPath === '/api/lamp/homekit') {
    return jsonRes(res, 200, { available: false, paired: false, name: 'BIG TUNA Lamp' });
  }

  // POST /api/lamp - public desired lamp state control
  if (req.method === 'POST' && urlPath === '/api/lamp') {
    try {
      const body = await parseBoundedJson(req);
      if (!body || typeof body !== 'object' || Array.isArray(body)
          || Object.keys(body).length !== 1 || typeof body.on !== 'boolean') {
      ...
      const { on, updatedAt } = writeLampState(body.on, 'public');
      return jsonRes(res, 200, { on, updatedAt });
    } catch (error) {
      if (error && error.code === 'BODY_TOO_LARGE') return jsonRes(res, 413, { error: error.message });
      if (error && error.code === 'INVALID_JSON') return jsonRes(res, 400, { error: error.message });
  ...
  // GET /api/lamp/device - ESP polling endpoint for desired state
  ...
  // POST /api/lamp/device/status - optional relay heartbeat/status
