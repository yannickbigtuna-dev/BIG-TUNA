...
test('Yannick Lights paid release retains Watch, widget, and control evidence', () => {
  const specPath = path.join(root, 'ios', 'app-factory', 'specs', 'big-tuna-lights.yml');
  const spec = JSON.parse(fs.readFileSync(specPath, 'utf8'));
  assert.doesNotThrow(() => validateSpec(spec));
  assert.equal(spec.app.name, 'Yannick Lights');
  assert.equal(spec.app.version, '1.3.0');
  assert.equal(spec.app.build, 6);
  assert.equal(spec.targets.watchMode, 'companion');
  assert.equal(spec.targets.watchWidgets, true);
  assert.equal(spec.targets.watchComplications, true);
  assert.equal(spec.targets.watchControls, true);
  assert.equal(spec.targets.watchConnectivity, true);
  assert.equal(spec.capabilities.watchConnectivity, true);
  assert.equal(spec.factory.bundleIdentifiers.watchApp, 'ca.yannickmorgans.bigtuna.lights.watchapp');
  assert.equal(spec.factory.bundleIdentifiers.watchExtension, null);
  assert.equal(spec.factory.bundleIdentifiers.watchWidget, 'ca.yannickmorgans.bigtuna.lights.watchapp.widget');
  assert.equal(spec.factory.bundleIdentifierHistory.watchApp, 'ca.yannickmorgans.bigtuna.lights.watchapp');
  assert.equal(spec.factory.bundleIdentifierHistory.watchWidget, 'ca.yannickmorgans.bigtuna.lights.watchapp.widget');
  ...
  assert.equal(targets.targetEvidence.watchWidgetExtension, 'ca.yannickmorgans.bigtuna.lights.watchapp.widget');
  assert.equal(targets.targetEvidence.watchControlExtension, 'ca.yannickmorgans.bigtuna.lights.watchapp.widget');
  const project = fs.readFileSync(path.join(output, 'project.yml'), 'utf8');
  assert.match(project, /^  YannickLights:\r?$/m);
  assert.match(project, /^  YannickLightsWidgets:\r?$/m);
  assert.match(project, /^  YannickLightsWatch:\r?$/m);
  assert.match(project, /^  YannickLightsWatchWidgets:\r?$/m);
  assert.match(project, /WKCompanionAppBundleIdentifier: ca\.yannickmorgans\.bigtuna\.lights/);
  const appSection = project.slice(project.indexOf('  YannickLights:'), project.indexOf('  YannickLightsWidgets:'));
  assert.match(appSection, /path: Shared/);
  const widgetSection = project.slice(project.indexOf('  YannickLightsWidgets:'), project.indexOf('  YannickLightsWatch:'));
  assert.match(widgetSection, /IPhoneWatchConnectivity\.swift/);
