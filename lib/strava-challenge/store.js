...
function mergeState(saved, base) {
  ...
  for (const key of ['invites', 'oauthStates', 'activities', 'weeks', 'scheduler', 'emailState', 'accountConnections', 'accountOAuthTransactions', 'accountActivities', 'websiteReviews', 'websiteNotificationEvents', 'challengeAccounts']) if (!merged[key] || typeof merged[key] !== 'object' || Array.isArray(merged[key])) merged[key] = base[key];
  return merged;
}
