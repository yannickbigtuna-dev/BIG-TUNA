...
function evaluateEmailRule(template, result = {}, participantRole = 'winner') {
  if (!template) return false;
  const rule = template.rule;
  if (!rule || !rule.condition || rule.condition === 'none') return true;

  const condition = String(rule.condition).trim();
  const param = Number(rule.param) || 0;
  const winnerId = result.winner;
  if (!winnerId) return false;
  const loserId = winnerId === 'yannick' ? 'emma' : 'yannick';
  const tie = result.winningMethod === 'activity_time_tiebreaker';

  const winnerActivities = (result[winnerId] && result[winnerId].qualifyingActivities) || 0;
  const loserActivities = (result[loserId] && result[loserId].qualifyingActivities) || 0;
  const activityDiff = Math.max(0, winnerActivities - loserActivities);

  const seasonBefore = result.seasonScoreBefore || {};
  const seasonAfter = result.seasonScoreAfter || {};
  const winnerSeasonBefore = Number(seasonBefore[winnerId]) || 0;
  const loserSeasonBefore = Number(seasonBefore[loserId]) || 0;
  const winnerSeasonAfter = Number(seasonAfter[winnerId]) || 0;
  const loserSeasonAfter = Number(seasonAfter[loserId]) || 0;
  const seasonDiffAfter = Math.max(0, winnerSeasonAfter - loserSeasonAfter);

  if (participantRole === 'winner') {
    switch (condition) {
      case 'margin_gt':
        return activityDiff > param;
      case 'overtake_season':
        return winnerSeasonBefore <= loserSeasonBefore && winnerSeasonAfter > loserSeasonAfter;
      case 'season_lead_gt':
        return seasonDiffAfter > param;
      case 'tiebreaker':
        return tie;
      default:
        return false;
    }
  } else if (participantRole === 'loser') {
    switch (condition) {
      case 'margin_gt':
        return activityDiff > param;
      case 'overtaken_season':
        return loserSeasonBefore >= winnerSeasonBefore && loserSeasonAfter < winnerSeasonAfter;
...
function selectCandidateFromPool(poolList, result, participantRole, isTie) {
  if (!Array.isArray(poolList) || !poolList.length) return null;
  const targetType = participantRole === 'winner'
    ? (isTie ? 'tiebreaker_win' : 'win')
    : (isTie ? 'tiebreaker_loss' : 'loss');
  const fallbackType = participantRole === 'winner' ? 'win' : 'loss';

  let candidates = poolList.filter(t => t && t.active !== false && t.type === targetType);
  if (!candidates.length && isTie) {
    candidates = poolList.filter(t => t && t.active !== false && t.type === fallbackType);
  }
  if (!candidates.length) return null;

  const matchingRuleCandidates = [];
  const generalCandidates = [];

  for (const t of candidates) {
    const hasRule = t.rule && t.rule.condition && t.rule.condition !== 'none';
    if (hasRule) {
      if (evaluateEmailRule(t, result, participantRole)) {
        matchingRuleCandidates.push(t);
      }
    } else {
      generalCandidates.push(t);
    }
  }

  if (matchingRuleCandidates.length > 0) {
    return matchingRuleCandidates[Math.floor(Math.random() * matchingRuleCandidates.length)];
  }
...
function renderWeeklyEmails(result = {}, options = {}) { const url = options.weekUrl || result.weekUrl || ''; const y = result.yannick || {}, e = result.emma || {}; const score = `${y.qualifyingActivities || 0}–${e.qualifyingActivities || 0}`; const season = result.seasonScoreAfter || options.seasonScoreAfter || {}; const seasonText = `Season score: Yannick ${season.yannick ?? 0} — ${season.emma ?? 0} Emma.`; const link = url ? `<p><a href="${escapeHtml(url)}" style="color:#93c5fd">View the week’s result</a></p>` : ''; const emails = {};
  ...
  if (poolList) {
    if (!winTemplate) {
      winTemplate = selectCandidateFromPool(poolList, result, 'winner', tie);
    }
    if (!lossTemplate) {
      lossTemplate = selectCandidateFromPool(poolList, result, 'loser', tie);
    }
  }
...
module.exports = { escapeHtml, formatDuration, interpolateTemplate, formatEmailBody, evaluateEmailRule, selectCandidateFromPool, renderConnectionEmail, renderWeeklyEmails, previewFixtures };
