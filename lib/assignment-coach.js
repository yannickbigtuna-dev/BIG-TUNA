...
function looksSubmitted(text) {
  ...
  if (/\b(no(?:t)? submissions?|not submitted|unsubmitted|submission required)\b/i.test(value)) return false;
  return /\b(submitted|submission complete|turned in|graded|completed)\b/i.test(value);
}
  ...
  return page.evaluate(() => {
    ...
    for (let index = 0; index < roots.length; index++) {
      ...
      for (const anchor of root.querySelectorAll('a[href]')) {
        ...
        const pinned = [item, root.host, hostItem, anchor].some(pinEvidence);
...
async function discoverPinnedCourseLinks(page, startUrl) {
  const links = (await deepLinks(page, { pinnedOnly: true }))
    .map(link => ({ title: link.text, href: link.href }))
...
async function discoverAssignmentPages(page, startUrl) {
  ...
  const explicit = (process.env.BRIGHTSPACE_ASSIGNMENT_PATHS || '')
    ...
    .filter(href => href && /(assignment|dropbox)/i.test(href));

  const byHref = new Map();
  for (const href of explicit) byHref.set(href, href);
  for (const link of links) {
    if (!sameOrigin(startUrl, link.href)) continue;
    if (/(assignment|assignments|dropbox)/i.test(link.text + ' ' + link.href)) {
