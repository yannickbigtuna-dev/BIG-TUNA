...
async function sendEmail(to, { subject, text, html, from, idempotencyKey }) {
  const sender = from || process.env.ASSIGNMENTS_FROM_EMAIL;
  ...
  if (idempotencyKey !== undefined && (!/^[A-Za-z0-9._:-]{1,256}$/.test(String(idempotencyKey)))) {
  ...
  return httpJson('https://api.resend.com/emails', {
    ...
    body: { from: sender, to: [to], subject, text, html },
  });
}
...
module.exports = {
  ...
  startScheduler,
  runDailyForAllUsers,
  // shared email sender (also used for account password-reset/test emails)
  sendEmail,
  // testing surface
  _test: {
    encryptSecret,
