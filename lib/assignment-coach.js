...
function getConfigStatus() {
  return {
    ...
    brightspace: Boolean(process.env.BRIGHTSPACE_ASSIGNMENTS_URL || process.env.BRIGHTSPACE_URL),
...
async function scrapeBrightspace() {
  const startUrl = process.env.BRIGHTSPACE_ASSIGNMENTS_URL || process.env.BRIGHTSPACE_URL;
  if (!startUrl) throw new Error('Set BRIGHTSPACE_ASSIGNMENTS_URL or BRIGHTSPACE_URL');
  ...
  try {
    ...
    if (process.env.BRIGHTSPACE_USERNAME && process.env.BRIGHTSPACE_PASSWORD) {
      await maybeLogin(page);
    }

    await new Promise(resolve => setTimeout(resolve, 2500));
    const currentUrl = page.url();
    const courseLinks = await discoverCourseLinks(page, startUrl);
    ...
    for (const course of targets.slice(0, Number(process.env.BRIGHTSPACE_MAX_COURSES) || 30)) {
      try {
        await page.goto(course.href, { waitUntil: 'networkidle2', timeout: 45000 });
        ...
        for (const assignmentPageUrl of pages.slice(0, 6)) {
          try {
            if (assignmentPageUrl !== page.url()) {
              await page.goto(assignmentPageUrl, { waitUntil: 'networkidle2', timeout: 45000 });
              await new Promise(resolve => setTimeout(resolve, 1200));
            }
            ...
            candidates.push({
              brightspaceId: normalizeId(course.href + '|assignment-page-error|' + assignmentPageUrl),
              sourceUrl: assignmentPageUrl,
              title: `Could not inspect assignments for ${course.title}`,
              course: course.title,
              dueAt: '',
              submitted: false,
        ...
        candidates.push({
          brightspaceId: normalizeId(course.href + '|course-error'),
          sourceUrl: course.href,
          title: `Could not inspect ${course.title}`,
          course: course.title,
          dueAt: '',
          submitted: false,
          instructions: err.message,
    ...
    for (const assignment of assignments.filter(a => !a.submitted && dueWithinWindow(a.dueAt)).slice(0, 12)) {
      ...
      try {
        await page.goto(assignment.sourceUrl, { waitUntil: 'networkidle2', timeout: 45000 });
        await new Promise(resolve => setTimeout(resolve, 1200));
...
async function maybeLogin(page) {
  ...
  await page.keyboard.type(process.env.BRIGHTSPACE_USERNAME, { delay: 10 });
  await page.click(passSelector, { clickCount: 3 });
  await page.keyboard.type(process.env.BRIGHTSPACE_PASSWORD, { delay: 10 });
...
async function runCheck({ manual = false } = {}) {
  ...
  try {
    const state = readState();
    const items = await scrapeBrightspace();
