...
function createChallengeAccounts({ dataDir, now = () => new Date(), notificationConfigured = false, getStravaStatus = () => ({ connected: false }), usernameForUserId = () => null, initialState, stateAdapter, encryptDeviceToken, decryptDeviceToken, sendPush, deviceTokenCipher, notificationSender } = {}) {
  ...
  return { getProfile, listChallenges, createChallenge, getChallenge, deleteChallenge, leaveChallenge, ingestActivities, updateSettings, updateTeam, createReviewRequest, listReviewRequests, listReviewInbox, decideReviewRequest, registerDevice, listNotificationEvents, createInvite, revokeInvite, previewInvite, acceptInvite, file, _readState: () => read(x => x), _mutate: mutate, isMember: member, canManage: roleCanManage, canDecideReview, evaluateActivity: evaluate };
}
