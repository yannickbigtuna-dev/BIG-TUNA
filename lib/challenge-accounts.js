...
function createChallengeAccounts({ dataDir, now = () => new Date(), notificationConfigured = false, getStravaStatus = () => ({ connected: false }), initialState, stateAdapter, encryptDeviceToken, decryptDeviceToken, sendPush, deviceTokenCipher, notificationSender } = {}) {
  ...
  return { getProfile, listChallenges, createChallenge, getChallenge, ingestActivities, updateSettings, createReviewRequest, listReviewRequests, listReviewInbox, decideReviewRequest, registerDevice, listNotificationEvents, file, _readState: () => read(x => x), _mutate: mutate, isMember: member, canManage: roleCanManage, canDecideReview, evaluateActivity: evaluate };
}
