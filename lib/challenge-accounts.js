...
const INVITE_CODE_RE = /^[A-Z]{6}$/;
const INVITE_LIFETIME_MS = 7 * 24 * 60 * 60 * 1000;
const PARTICIPANT_TEAMS = new Set(['red', 'blue']);
...
function normalizeInvitationCode(code) {
  ...
  // Invitation codes are six ASCII letters.  Trim only surrounding whitespace:
  // accepting embedded whitespace changes the credential and Unicode case
  // mapping can expand a single input character into several ASCII letters.
  const trimmed = code.trim();
  return /^[A-Za-z]{6}$/.test(trimmed) ? trimmed.toUpperCase() : null;
}

const TEMPLATES = Object.freeze({
  'yannick-emma-default': { name: 'Yannick & Emma Challenge', participants: [{ userId: 'yannick', role: 'owner', team: 'red' }, { userId: 'emma', role: 'member', team: 'blue' }], qualifyingActivities: ['Run', 'Ride', 'Walk', 'Swim'], thresholds: { distanceMeters: 1000 }, scoring: { mode: 'count', pointsPerActivity: 1 }, cadence: { type: 'weekly' }, timezone: 'America/Halifax', manualReview: true },
...
function normalizeParticipants(value, actorId, existing) {
  ...
  const existingById = new Map((existing || []).map((entry, index) => [entry.userId, PARTICIPANT_TEAMS.has(entry.team) ? entry.team : (index % 2 ? 'blue' : 'red')]));
  const seen = new Set(); const result = source.map((entry, index) => {
    ...
    seen.add(uid); return { userId: uid, role, team: existingById.get(uid) || (index % 2 ? 'blue' : 'red') };
  });
...
function safeActivity(a, rules) { return { id: a.id, participantID: a.userId, sportType: text(a.sportType || a.type, 40) || null, name: text(a.name, 160), startDate: a.startDate, distanceMeters: number(a.distanceMeters ?? a.distance), movingTime: number(a.movingTime ?? a.durationSeconds), qualifies: evaluate(a, rules), reviewState: a.reviewState || null }; }
...
function publicChallenge(challenge, includeDetail = false, currentAt = new Date(), usernameForUserId = () => null) {
  // This is called only after membership has been checked. Resolve names at
  // serialization time so username changes do not leave stale copies in state.
  const participant = (p, index) => {
    const username = text(usernameForUserId(p.userId), 80);
    let team = PARTICIPANT_TEAMS.has(p.team) ? p.team : (index % 2 ? 'blue' : 'red');
    if (challenge.template === 'yannick-emma-default') {
      const identity = String(username || p.userId).trim().toLowerCase();
      if (identity === 'yannick') team = 'red';
      if (identity === 'emma' || identity === 'fishyemma') team = 'blue';
    }
    return { userId: p.userId, username, role: p.role, team };
  };
  const base = { id: challenge.id, name: challenge.name, template: challenge.template, createdAt: challenge.createdAt, updatedAt: challenge.updatedAt, participants: challenge.participants.map(participant), rules: clone(challenge.rules) };
...
function createChallengeAccounts({ dataDir, now = () => new Date(), notificationConfigured = false, getStravaStatus = () => ({ connected: false }), usernameForUserId = () => null, initialState, stateAdapter, encryptDeviceToken, decryptDeviceToken, sendPush, deviceTokenCipher, notificationSender } = {}) {
  ...
  const publicView = (challenge, includeDetail = false) => publicChallenge(challenge, includeDetail, now(), usernameForUserId);
  ...
  function normalizeAdapterState(accountState, mutate = true) {
    if (!accountState || typeof accountState !== 'object' || Array.isArray(accountState)) fail('Challenge account state is unavailable.', 'state_unavailable', 500);
    const base = defaults();
    if (!mutate) return { ...base, ...accountState };
    for (const [key, value] of Object.entries(base)) if (accountState[key] === undefined) accountState[key] = clone(value);
    return accountState;
  }
  function mutate(fn) { const run = chain.then(async () => {
    if (stateAdapter) return stateAdapter.mutate(async accountState => { const target = normalizeAdapterState(accountState); const result = await fn(target); target.version = 1; return clone(result); });
    const result = await fn(load()); write(); return clone(result);
  }); chain = run.catch(() => {}); return run; }
  const read = fn => stateAdapter ? clone(stateAdapter.read(accountState => fn(normalizeAdapterState(accountState, false)))) : clone(fn(load()));
  ...
  function invitationForCredential(s, input) {
    const token = input && input.token;
    const code = input && input.code;
    const digest = token ? invitationDigest(token) : invitationDigest(code);
    for (const challenge of Object.values(s.challenges || {})) {
      const invite = challenge.invite;
      if (invite && ((token && sameDigest(invite.tokenHash, digest)) || (code && sameDigest(invite.codeHash, digest)))) return { challenge, invite };
    }
    fail('Invitation is unavailable.', 'invite_unavailable', 404);
  }
  function assertUsableInvitation(challenge, invite, currentTime) {
    if (invite.revokedAt) fail('Invitation is unavailable.', 'invite_unavailable', 404);
    const expiry = Date.parse(invite.expiresAt);
    if (!Number.isFinite(expiry) || expiry <= currentTime.getTime()) fail('Invitation is unavailable.', 'invite_unavailable', 404);
    if (challenge.template === 'yannick-emma-default') fail('Invitation is unavailable.', 'invite_unavailable', 404);
  }
  async function createInvite(user, challengeId) {
    ...
    const result = await mutate(s => {
      ...
      for (let attempt = 0; attempt < 32; attempt++) {
        const candidate = Array.from({ length: 6 }, () => String.fromCharCode(65 + crypto.randomInt(26))).join('');
        const candidateHash = invitationDigest(candidate);
        const collision = Object.values(s.challenges || {}).some(challenge => challenge.invite && sameDigest(challenge.invite.codeHash, candidateHash));
        if (!collision) { code = candidate; break; }
      }
      if (!code) fail('Invitation could not be created.', 'invite_unavailable', 503);
      const createdAt = iso(now());
      const expiresAt = iso(new Date(Date.parse(createdAt) + INVITE_LIFETIME_MS));
      // A replacement record revokes both previous credentials atomically.
      c.invite = { tokenHash: invitationDigest(token), codeHash: invitationDigest(code), createdAt, expiresAt, createdBy: u.id, revokedAt: null };
  ...
  async function previewInvite(input = {}) {
    const credential = normalizeInvitationCredential(input);
    return read(s => {
      const { challenge, invite } = invitationForCredential(s, credential);
  ...
  async function acceptInvite(user, input = {}) {
    ...
    const credential = normalizeInvitationCredential(input);
    return mutate(s => {
      const { challenge: c, invite } = invitationForCredential(s, credential);
      assertUsableInvitation(c, invite, now());
      const existing = member(c, u.id);
      if (existing) return { challenge: publicView(c, true), alreadyMember: true };
      if (c.participants.length >= 50) fail('This challenge is full.', 'invite_full', 409);
      const joinedAt = iso(now());
      c.participants.forEach((participant, index) => {
        if (!PARTICIPANT_TEAMS.has(participant.team)) participant.team = index % 2 ? 'blue' : 'red';
      });
      c.participants.push({ userId: u.id, role: 'member', team: c.participants.length % 2 ? 'blue' : 'red' });
      ...
      return { challenge: publicView(c, true), alreadyMember: false };
    });
  }
  function normalizeInvitationCredential(input) {
    if (!plainObject(input) || Object.keys(input).length !== 1) fail('Invitation is unavailable.', 'invite_unavailable', 404);
    if (own(input, 'token') && validInvitationToken(input.token)) return { token: input.token };
    if (own(input, 'code')) {
  ...
  async function listChallenges(user) { const u = actor(user); return read(s => Object.values(s.challenges).filter(c => member(c, u.id)).sort((a,b) => String(b.updatedAt).localeCompare(String(a.updatedAt))).map(c => publicView(c))); }
  async function createChallenge(user, input = {}) { const u = actor(user); if (!plainObject(input)) fail('Challenge input is invalid.', 'invalid_challenge', 400); const template = input.template || 'custom'; if (!TEMPLATE_NAMES.has(template)) fail('Challenge template is invalid.', 'invalid_template', 400); const requestKey = input.idempotencyKey === undefined ? null : text(input.idempotencyKey, 128); if (input.idempotencyKey !== undefined && (!requestKey || !/^[A-Za-z0-9_-]{16,128}$/.test(requestKey))) fail('Challenge creation key is invalid.', 'invalid_challenge', 400); const preset = clone(TEMPLATES[template]); const suppliedParticipants = input.participants;
    ...
    if (creatorIndex >= 0 && template !== 'yannick-emma-default') { participants.splice(creatorIndex, 1); participants.unshift({ userId: u.id, role: 'owner' }); }
    else if (creatorIndex >= 0) participants[creatorIndex] = { userId: u.id, role: 'owner' };
    ...
    return mutate(s => {
      ...
      if (existingID && s.challenges[existingID]) return publicView(s.challenges[existingID]);
      ...
      return publicView(challenge);
    }); }
  async function getChallenge(user, challengeId) { const u = actor(user); return read(s => { const c = s.challenges[challengeId]; requireMember(c, u); return publicView(c, true); }); }
  ...
  async function updateSettings(user, challengeId, input = {}) {
    const u = actor(user);
    if (!plainObject(input)) fail('Challenge settings are invalid.', 'invalid_challenge', 400);
    return mutate(s => {
      const c = s.challenges[challengeId];
      requireManager(c, u);
      const normalized = normalizeRules(input, { ...c.rules, participants: c.participants }, u.id);
      if (member(c, u.id).role !== 'owner') {
        const currentOwners = c.participants.filter(p => p.role === 'owner').map(p => p.userId).sort();
        const requestedOwners = normalized.participants.filter(p => p.role === 'owner').map(p => p.userId).sort();
        if (currentOwners.length !== requestedOwners.length || currentOwners.some((id, index) => id !== requestedOwners[index])) {
          fail('Only the challenge owner can change challenge ownership.', 'owner_change_forbidden', 403);
        }
      }
      c.participants = normalized.participants;
      c.rules = { qualifyingActivities: normalized.qualifyingActivities, sportRules: normalized.sportRules, thresholds: normalized.thresholds, scoring: normalized.scoring, cadence: normalized.cadence, timezone: normalized.timezone, manualReview: normalized.manualReview };
      if (input.name !== undefined) {
        const name = text(input.name, 120);
        if (!name) fail('Challenge name is invalid.', 'invalid_challenge', 400);
        c.name = name;
      }
      c.updatedAt = iso(now());
      c.audit.push({ at: c.updatedAt, type: 'settings_updated', actorId: u.id });
      c.history.push({ at: c.updatedAt, type: 'settings_updated', score: Object.fromEntries(c.participants.map(p => [p.userId, scoreFor(Object.values(c.activities).filter(a => a.userId === p.userId), c.rules)])) });
      return publicView(c, true);
    });
  }
  async function updateTeam(user, challengeId, input = {}) {
    const u = actor(user);
    if (!plainObject(input) || Object.keys(input).length !== 1 || !PARTICIPANT_TEAMS.has(input.team)) fail('Team must be red or blue.', 'invalid_team', 400);
    return mutate(s => {
      const c = s.challenges[challengeId];
      requireMember(c, u);
      if (c.template === 'yannick-emma-default') fail('The fixed Yannick & Emma teams cannot be changed.', 'fixed_team_forbidden', 403);
      c.participants.forEach((participant, index) => {
        if (!PARTICIPANT_TEAMS.has(participant.team)) participant.team = index % 2 ? 'blue' : 'red';
      });
      const selected = member(c, u.id);
      selected.team = input.team;
      if (c.participants.length === 2) {
        const peer = c.participants.find(participant => participant.userId !== u.id);
        peer.team = input.team === 'red' ? 'blue' : 'red';
      }
      c.updatedAt = iso(now());
      c.audit ||= [];
      c.audit.push({ at: c.updatedAt, type: 'team_updated', actorId: u.id, team: input.team });
      return publicView(c, true);
    });
  }
  ...
  async function leaveChallenge(user, challengeId) {
    ...
    return mutate(s => {
      const c = s.challenges[challengeId];
      requireMember(c, u);
      if (c.template === 'yannick-emma-default') fail('The fixed Yannick & Emma challenge cannot be left.', 'fixed_challenge_leave_forbidden', 403);
      ...
      // Participants are authoritative at the challenge level.  Remove a
      // noncanonical copy that an early invitation build may have persisted.
      if (c.rules) delete c.rules.participants;
      ...
      for (const [reviewId, review] of Object.entries(c.reviews || {})) {
        const activity = c.activities && c.activities[review.activityId];
        if (review.requesterId === u.id || !activity) { delete c.reviews[reviewId]; removedReviewIds.add(reviewId); }
      }
      for (const [eventId, event] of Object.entries(s.notificationEvents || {})) {
        if (event.challengeId === challengeId && (event.recipientId === u.id || removedReviewIds.has(event.reviewId))) delete s.notificationEvents[eventId];
      }
  ...
  async function sendTestNotification(user) {
    const u = actor(user);
    const devices = read(s => Object.values(s.devices || {}).filter(d => d.userId === u.id && !d.disabledAt));
    if (!devices.length) {
      return {
        sent: false,
        count: 0,
        devicesCount: 0,
        apnsConfigured: Boolean(notificationConfigured),
        reason: 'no_registered_devices',
        message: 'No registered devices found for your account. Please enable notifications in the app settings first.'
      };
    }
    if (typeof pushSender !== 'function' || typeof decryptToken !== 'function') {
      return {
        ...
        message: 'Push notification service is temporarily unavailable on the server.'
      };
    }
    let sentCount = 0;
    for (const device of devices) {
      try {
        ...
        const result = await pushSender({
          ...
          event: {
            ...
            reviewId: null,
            title: 'BIG TUNA Test Notification',
            body: 'Push notifications are working for BIG TUNA!'
          }
        });
        if (result && (result.invalidToken || result.reason === 'invalid_token')) {
          await mutate(s => {
            const d = s.devices[`${device.userId}:${device.fingerprint}`];
            if (d) d.disabledAt = iso(now());
          });
        } else if (!result || (result.sent !== false && result.delivery !== 'failed')) {
          sentCount++;
        }
      } catch {}
    }
    return {
      ...
      devicesCount: devices.length,
      apnsConfigured: Boolean(notificationConfigured),
      reason: sentCount > 0 ? null : (notificationConfigured ? 'delivery_failed' : 'apns_unconfigured'),
      message: sentCount > 0
        ? `Test notification sent to ${sentCount} device${sentCount === 1 ? '' : 's'}.`
        : (notificationConfigured
            ? 'Test notification could not be delivered by APNs. Check device connection or token.'
            : 'Device is registered, but Apple APNs credentials (AuthKey .p8) are not configured on the server yet.')
    };
  }
  async function getNotificationStatus(user) {
    ...
    return {
      ...
      apnsConfigured: Boolean(notificationConfigured),
      encryptionConfigured: typeof encryptToken === 'function',
    };
  }
  return { getProfile, listChallenges, createChallenge, getChallenge, deleteChallenge, leaveChallenge, ingestActivities, updateSettings, updateTeam, createReviewRequest, listReviewRequests, listReviewInbox, decideReviewRequest, registerDevice, listNotificationEvents, sendTestNotification, getNotificationStatus, createInvite, revokeInvite, previewInvite, acceptInvite, file, _readState: () => read(x => x), _mutate: mutate, isMember: member, canManage: roleCanManage, canDecideReview, evaluateActivity: evaluate };
}
