...
function normalizeRules(input = {}, base = {}, actorId) {
  ...
  return { qualifyingActivities, thresholds, scoring: { mode, pointsPerActivity }, cadence: { type: cadenceType }, timezone, manualReview: input.manualReview === undefined ? !!base.manualReview : !!input.manualReview, participants: normalizeParticipants(input.participants, actorId, base.participants) };
}
function evaluate(activity, rules) {
  const type = text(activity.sportType || activity.type, 40) || ''; const t = rules.thresholds || {};
  const base = rules.qualifyingActivities.includes(type) &&
...
function createChallengeAccounts({ dataDir, now = () => new Date(), notificationConfigured = false, getStravaStatus = () => ({ connected: false }), initialState } = {}) {
  if (!dataDir && !initialState) fail('A challenge account data directory is required.', 'invalid_configuration', 500);
  const file = dataDir && path.join(dataDir, 'state.json'); let state; let chain = Promise.resolve();
  const defaults = () => ({ version: 1, challenges: {}, devices: {}, notificationEvents: {} });
  ...
  function mutate(fn) { const run = chain.then(async () => { const result = await fn(load()); write(); return clone(result); }); chain = run.catch(() => {}); return run; }
  const read = fn => clone(fn(load()));
  ...
  function recordEvent(s, challenge, type, recipientId, data = {}) { const event = { id: id('event'), challengeId: challenge.id, type, recipientId, reviewId: data.reviewId || null, createdAt: iso(now()), delivery: notificationConfigured ? 'pending' : 'stored' }; s.notificationEvents[event.id] = event; return event; }
  ...
  async function createChallenge(user, input = {}) { const u = actor(user); if (!plainObject(input)) fail('Challenge input is invalid.', 'invalid_challenge', 400); const template = input.template || 'custom'; if (!TEMPLATE_NAMES.has(template)) fail('Challenge template is invalid.', 'invalid_template', 400); const preset = clone(TEMPLATES[template]); const suppliedParticipants = input.participants;
    ...
    return mutate(s => { const rules = normalizeRules(merged, preset, u.id); const name = text(merged.name, 120); if (!name) fail('Challenge name is required.', 'invalid_challenge', 400); const challenge = { id: id('challenge'), name, template, createdAt: iso(now()), updatedAt: iso(now()), participants: rules.participants, rules: { qualifyingActivities: rules.qualifyingActivities, thresholds: rules.thresholds, scoring: rules.scoring, cadence: rules.cadence, timezone: rules.timezone, manualReview: rules.manualReview }, activities: {}, reviews: {}, audit: [{ at: iso(now()), type: 'challenge_created', actorId: u.id }], history: [] }; s.challenges[challenge.id] = challenge; return publicChallenge(challenge); }); }
  ...
  async function decideReviewRequest(user, challengeId, reviewId, input = {}) { const u = actor(user); if (!plainObject(input)) fail('Review decision is invalid.', 'invalid_decision', 400); const decision = input.decision; if (!['approve', 'reject'].includes(decision)) fail('Review decision is invalid.', 'invalid_decision', 400); return mutate(s => { const c = s.challenges[challengeId]; requireManager(c, u); const review = c.reviews[reviewId]; if (!review) fail('Review request not found.', 'not_found', 404); if (review.requesterId === u.id) fail('A requester cannot decide their own review.', 'self_approval_forbidden', 403); if (review.status !== 'pending') { if ((review.status === 'approved') === (decision === 'approve')) return { review: safeReview(review), challenge: publicChallenge(c, true, now()), idempotent: true }; fail('Review request has already been decided.', 'review_decided', 409); }
    review.status = decision === 'approve' ? 'approved' : 'rejected'; review.decidedAt = iso(now()); review.decidedBy = { id: u.id, username: u.username }; review.decisionReason = text(input.reason, 500); const activity = c.activities[review.activityId]; if (activity) { activity.manualQualification = decision === 'approve'; activity.reviewState = review.status; } const scores = Object.fromEntries(c.participants.map(p => [p.userId, scoreFor(Object.values(c.activities).filter(a => a.userId === p.userId), c.rules)])); c.history.push({ at: review.decidedAt, type: `review_${review.status}`, score: scores }); c.audit.push({ at: review.decidedAt, type: `review_${review.status}`, actorId: u.id, reviewId: review.id, activityId: review.activityId }); c.updatedAt = review.decidedAt; recordEvent(s, c, `review_${review.status}`, review.requesterId, { reviewId: review.id }); return { review: safeReview(review), challenge: publicChallenge(c, true, now()), idempotent: false }; }); }
  async function registerDevice(user, input = {}) { const u = actor(user); if (!plainObject(input)) fail('Device input is invalid.', 'invalid_device', 400); const token = text(input.token || input.endpoint, 4096); const platform = text(input.platform, 40) || 'unknown'; if (!token) fail('A device subscription is required.', 'invalid_device', 400); return mutate(s => { const fingerprint = crypto.createHash('sha256').update(token).digest('hex'); const key = `${u.id}:${fingerprint}`, existing = s.devices[key]; s.devices[key] = { id: existing?.id || id('device'), userId: u.id, fingerprint, platform, createdAt: existing?.createdAt || iso(now()), updatedAt: iso(now()) }; return { id: s.devices[key].id, platform, registered: true }; }); }
  ...
  return { getProfile, listChallenges, createChallenge, getChallenge, ingestActivities, updateSettings, createReviewRequest, listReviewRequests, decideReviewRequest, registerDevice, listNotificationEvents, file, _readState: () => read(x => x), _mutate: mutate, isMember: member, canManage: roleCanManage, evaluateActivity: evaluate };
}
