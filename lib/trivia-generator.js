...
function selectFallbackQuestions({ topic = '', difficulty = 'any', count = 1, exclude = [] } = {}) {
  ...
  const matched = difficultyPool.filter(question => questionMatchesTopic(question, topic));
  const unmatched = difficultyPool.filter(question => !questionMatchesTopic(question, topic));
  shuffleInPlace(matched);
  shuffleInPlace(unmatched);

  return {
    topicMatched: !topic || matched.length > 0,
    questions: validateTriviaQuestions([...matched, ...unmatched], safeCount, excludeSet),
  };
}
