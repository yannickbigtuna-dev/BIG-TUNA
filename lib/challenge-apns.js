...
function createChallengeApns({ env = process.env, logger = console, now } = {}) {
  ...
  const primaryHost = text(env.APNS_ENVIRONMENT).toLowerCase() === 'sandbox'
    ? 'https://api.sandbox.push.apple.com'
    : 'https://api.push.apple.com';
  const alternateHost = primaryHost === 'https://api.sandbox.push.apple.com'
    ? 'https://api.push.apple.com'
    : 'https://api.sandbox.push.apple.com';
  function safeWarn(message) { if (logger && typeof logger.warn === 'function') logger.warn(`[challenge apns] ${message}`); }

  async function postToHost(host, token, payload) {
    let client;
    try {
  ...
  async function send({ token, eventType, challengeId, reviewId, title, body }) {
    ...
    const payload = JSON.stringify({
      aps: { alert: { title: text(title).slice(0, 100), body: text(body).slice(0, 256) }, sound: 'default' },
      eventType: text(eventType).slice(0, 64), challengeId: text(challengeId).slice(0, 96), reviewId: text(reviewId).slice(0, 128),
      title: text(title).slice(0, 100), body: text(body).slice(0, 256),
    });
    try {
      let res = await postToHost(primaryHost, token, payload);
      // If primary gateway rejected due to bad device token or environment mismatch, try alternate gateway
      const isMismatch = res.status === 400 || (res.status === 403 && (res.reason === 'BadEnvironmentKeyInToken' || res.reason === 'BadEnvironmentKeyIdInToken'));
      if (isMismatch) {
        try {
          const altRes = await postToHost(alternateHost, token, payload);
          if (altRes.status >= 200 && altRes.status < 300) return { delivery: 'sent' };
          if (res.status === 400 && altRes.status > 0) res = altRes;
        } catch {}
      }
      if (res.status >= 200 && res.status < 300) return { delivery: 'sent' };
      if ([400, 404, 410].includes(res.status) || res.reason === 'BadDeviceToken' || res.reason === 'Unregistered' || res.reason === 'ExpiredToken') {
        return { delivery: 'failed', reason: res.reason || 'invalid_token' };
      }
      return { delivery: 'failed', reason: res.reason || 'provider_unavailable' };
    } catch (error) {
      safeWarn('delivery unavailable: ' + (error && error.message));
