...
function createChallengeApns({ env = process.env, logger = console, now } = {}) {
  ...
  async function send({ token, eventType, challengeId, reviewId, title, body }) {
    ...
    try {
      let status = await postToHost(primaryHost, token, payload);
      if (status === 400) {
        try {
          const altStatus = await postToHost(alternateHost, token, payload);
          if (altStatus >= 200 && altStatus < 300) return { delivery: 'sent' };
          status = altStatus;
        } catch {}
      }
      if (status >= 200 && status < 300) return { delivery: 'sent' };
      if ([400, 404, 410].includes(status)) return { delivery: 'failed', reason: 'invalid_token' };
