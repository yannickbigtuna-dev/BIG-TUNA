...
- BIG TUNA Lights 1.1.2 (4) is the first explicitly iPhone-only release, created after Sideloadly rejected the embedded Watch companion identifier. Its transient Sideloadly-prepared artifact must contain exactly the host app and one iOS `.appex`, with no `Watch/` payload. Record its successful CI run/hash only after the new archive passes inspection; physical signing/install results remain a separate device gate.
