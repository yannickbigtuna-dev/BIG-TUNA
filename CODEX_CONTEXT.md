...
`start-everything.ps1` is the canonical local orchestrator. It starts or restarts PM2 apps `apps-server` and `mcp-server`, saves the PM2 process list, starts `cloudflared.exe` with `cloudflared-config.yml` only when the tunnel process is absent, and starts `auto-update.ps1` hidden when the git reloader is absent. `auto-update.ps1` polls `origin/main` every 10 seconds, but only pulls/restarts when `origin/main` is a strict fast-forward descendant of local `HEAD`; a locally-ahead checkout is left running until it is pushed, and divergence is reported without repeated restart attempts.
...
`POST /api/trivia/generate` is authenticated and powers the Lumina Trivia game. Body remains `{ topic?: string, count?: number, model?: string, difficulty?: 'easy'|'medium'|'hard', exclude?: string[] }`. `topic` is trimmed/clamped to 100 chars; `count` is clamped to 1–10 (default 5); `difficulty` pins returned questions (anything else => mixed); and `exclude` keeps the 80 most-recent question texts (each clamped to 140 chars) for bank/prompt dedupe. A blank topic is the only path allowed to use the validated built-in safety bank (`lib/trivia-generator.js`, 216 questions including generated capital variants): it returns immediately with `source:'built-in'` and never queues Ollama. Every nonblank topic is isolated in an in-memory AI bank keyed by normalized topic+difficulty+requested-model; a ready question is returned from that exact bank, while a cache miss coalesces concurrent callers around one one-question Ollama fill under a single 9-second server deadline. Custom-topic generation never falls back to random built-ins: success is `source:'ai-topic'`/`topicMatched:true`, and an unavailable, timed-out, or invalid generation returns 503 with `questions:[]`. After a custom question is consumed, the next one-question fill starts immediately so later calls can consume or share it. AI generation uses a compact `q/a/c/g/d/e` JSON schema, JSON Schema structured output, `num_ctx:2048`, bounded prediction, `keep_alive:"30m"`, and `pickTriviaModel()` (preferring the lower-latency `qwen2.5:1.5b`, then `phi4-mini:latest`, unless an installed model was explicitly requested). Malformed/placeholder answers are rejected and accepted questions are answer-shuffled. Response metadata includes `source`, `aiAvailable`, `refillPending`, and `topicMatched`; valid questions use `{ id, question, answers:[4], correct, category, difficulty, explanation }`.
...
- Questions are supplied through `POST /api/trivia/generate`. A blank **Topic** means an immediate random mix from the built-in validated bank; every trimmed nonblank Topic, including one- and two-character values, requires newly generated questions from local Ollama and can never consume the random built-in cache. Difficulty is selected independently (Mixed / Easy / Medium / Hard).
...
- **Ready-ahead generation:** the home screen fills a keyed `PREBANK`=4. Blank-topic requests have a short 2.5-second browser budget and may return a batch immediately; custom-topic requests produce one question at a time, have 9.5 seconds of browser headroom around the server's 9-second generation deadline, and keep filling the bank as fast as each response permits. Start immediately drains any question already ready; otherwise it shares `fetching[key]`, wakes on the first bank arrival instead of awaiting all four, and renders under a strict 9.75-second click-to-first-question deadline while the remaining prebank continues. During play, `ensureBuffer()` keeps `PREFETCH`=3 ready ahead, drains and shares the same keyed prebank pipeline before making a direct request, and is called on render and immediately after an answer is recorded. Bank/run state remains keyed by topic **and** difficulty.
- **Bank warm-up (`warmHomeBank`):** the home-screen bank (current Topic text + Difficulty) is (re-)warmed on load (in parallel with the `/api/eco-ai/status` check), on Difficulty change, ~450ms after the player pauses on every nonblank trimmed Topic (including one or two characters), immediately when Topic is cleared, and whenever the player returns home (quit/home/tab-to-Play).
...
- **No duplicates:** the client remembers every question served (`run.seen`, normalised) and sends up to 80 recent texts (`EXCLUDE_MAX`, matching server `TRIVIA_MAX_EXCLUDE`) on each request; the server uses the full list for instant bank/safety-bank selection and trims the AI prompt internally. The single client `addUnique()` choke-point still drops any repeat before display.
- Per-user stats/history persist via `Auth.saveSettings('trivia', …)` (best score, games, totals, last 25 runs incl. difficulty) with a `localStorage` fallback; the in-app Rankings tab ranks the user's own completed runs by points. If Ollama is offline the home screen explains that typed topics cannot start; clearing Topic still allows the blank-topic built-in random mix.
