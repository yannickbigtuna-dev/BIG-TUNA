...
A temporary "send test email" button (`#test-email-btn`, `apps/index.html`) sits in the homepage corner-tools next to the Lights button; it's hidden until `Auth.onReady` fires for the `yannick` account specifically (same convention as `/api/lights` and the terminal WebSocket — checked both client-side, for visibility, and server-side in `/api/account/test-email`, for enforcement), calls `POST /api/account/test-email`, and flashes the icon `--success`/`--danger` based on the result. It's marked `TEMP` in both the HTML and script and is meant to be deleted once Resend delivery is confirmed working in production.
...
- **No duplicates:** the client remembers every question served (`run.seen`, normalised) and sends the recent set as `exclude` on each request; the single `addUnique()` choke-point drops any repeat before it can be shown, and the server dedupes again against the same `exclude` list. This is the intended architecture for "remembering" asked questions.
