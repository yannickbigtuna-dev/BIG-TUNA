...
- Native surfaces use owner-authenticated `GET/PUT /api/lights/native/v1`, explicit physical target state, and an idempotent command ID. They never optimistically cache an unconfirmed change. The website, HomeKit, and ESP routes retain their prior stored/inverted contracts.
