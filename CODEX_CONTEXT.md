...
The former temporary "send test email" corner-tools button was removed once Resend delivery was confirmed; that same homepage corner-tools slot (`#admin-dashboard-btn`, `apps/index.html`, next to the Lights button) now links to `/admin/` instead, hidden until `Auth.onReady` fires for the `yannick` account specifically (the same owner-only convention as the terminal WebSocket). This is a second, homepage-level entry point to the admin dashboard alongside the hidden dropdown link in `apps/auth.js`'s account widget — see "Admin Dashboard & Email Campaigns" above. `POST /api/account/test-email` still exists server-side (harmless, still yannick-gated) but nothing in the UI calls it anymore.
...
- The iPhone app and WidgetKit extension use App Group `group.ca.yannickmorgans.bigtuna.lights` for separate revocable Lights- and Lamp-scoped bearer tokens plus last confirmed native states. Existing Lights-only installations can exchange that scoped token once for Lamp access. WatchConnectivity transfers only the Lights token and confirmed Light state to the Watch App Group; no password is transferred or persisted. Login briefly uses a normal website session, exchanges it for both scoped credentials, then revokes the website session.
- Native surfaces use owner-authenticated `GET/PUT /api/lights/native/v1`, explicit physical target state, and an idempotent command ID. They never optimistically cache an unconfirmed change. The website and HomeKit retain their stored/inverted contracts; the ESP device endpoint passes the stored relay command through unchanged and firmware applies relay polarity.
...
`sendCampaign(campaignId)` resolves `recipientIds` against `data/users.json`, **skips any recipient with no `email` on file** (recorded as `status:'skipped'` in `sendResults`, surfaced in the UI rather than silently under-delivering — email is optional/nullable on this site's user schema), and sends **sequentially** with a ~400ms delay between recipients (fine at this site's scale, avoids Resend rate limits without a concurrency pool). `sendResults[]` is written back incrementally after each send so a mid-send crash doesn't lose progress. A campaign scheduler (`startCampaignScheduler`, wired into `server.listen()`) runs at a 60-second interval, picking up `status:'scheduled'` campaigns whose `scheduledAt` has passed, guarded by an in-memory in-flight `Set` so a slow send can't be double-triggered.
...
`GET /api/lights` is public and returns `{ on, updatedAt }`. `GET /api/lights/events` is a public Server-Sent Events stream that immediately emits the same desired state payload whenever it changes. `POST /api/lights` is public and accepts only a JSON body shaped exactly as `{ on: boolean }`; malformed bodies or non-Boolean `on` values are rejected. Device routes preserve legacy unauthenticated polling only when `LIGHTS_DEVICE_API_TOKEN` is unset, but those calls never create trusted telemetry. Once the same secret is configured in the server and ESP, both device routes require `X-Big-Tuna-Device-Token`; only authenticated polls/reports feed the website/native device indicators. `GET /api/lights/device` returns the stored `on` relay command unchanged with `pollAfterMs: 250`; ESP firmware applies the relay-polarity inversion. `GET /api/lights/device/status` returns the trusted `{ on, receivedAt, polledAt, recentlyPolled, recentWindowMs }` view.
...
translate through the existing website/HomeKit storage boundary without changing
legacy payloads; the device endpoint passes the stored relay command to ESP firmware unchanged.

`hap-nodejs` also starts a LAN-only HomeKit bridge named `BIG TUNA Lights` on TCP 51826, advertised by mDNS only through the physical Wi-Fi IPv4 address (explicit `HOMEKIT_BIND_ADDRESS`, otherwise the first IPv4 on `HOMEKIT_BIND_INTERFACE`, default `Wi-Fi`) so Tailscale and IPv6 routes cannot break Apple Home pairing. It translates HomeKit's physical-light `On` value through the existing website storage boundary and uses the same `writeLightsState` path; the ESP device endpoint then passes that stored relay command unchanged to firmware, which owns relay polarity. Pairing data lives in `data/lights/homekit/`; `GET /api/lights/homekit` returns status only to the authenticated `yannick` account, while the owner-gated `/api/lights/homekit/qr` returns a locally generated, no-store SVG from the HAP setup URI only before pairing. The Cloudflare Tunnel is not part of HomeKit discovery; see `docs/apple-home-lights.md` for the QR pairing/firewall guide.
...
`GET/PUT /api/lamp/native/v1` is the owner-only Apple-client contract for Lamp, mirroring the Lights native v1 contract (`physicalOn`, `reportedPhysicalOn`, `recentlyPolled`, `updatedAt`, `revision`, bounded `commandId` idempotency). `POST/DELETE /api/lamp/native/v1/session` manages scoped bearer sessions. Session creation accepts either the owner's temporary website session or an existing valid owner Lights-scoped token, allowing pre-Lamp app installations to bootstrap Lamp access without storing or re-entering the account password.
...
- Chrome-free, text-free UI: just the light switch on its plate. There is no sign-in screen; public visitors can control the light through the legacy web route.
- The plate's lower screw turns green (`body.is-polled`) when `/api/lights/device/status` reports `recentlyPolled` (device polled within the last 5 seconds); the upper screw is decorative because web control is public.
- Reads `/api/lights` for state, inverts that API value client-side to match the Arduino-driven physical light state, and posts the inverse value back when toggled. `POST /api/lights` is public and strictly accepts `{ on: boolean }`.
...
- There is no server-side Lights schedule. State changes are made manually through the public website, HomeKit, or native controls; the ESP relay applies the latest desired state when it polls.
...
- ESP8266 relay integration should poll `/api/lights/device`, respect the returned `pollAfterMs` hint when practical, apply the returned stored `on` command using its own relay-polarity handling, and keep last known relay state if the website is temporarily unreachable. The server does not invert the device command.
