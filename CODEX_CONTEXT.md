...
- Uses browser geolocation on first load when available, otherwise falls back to Halifax. Saved searched cities, last selected location, and preferred weather data source are stored in `localStorage` under `weather_locations`, `weather_last_location`, and `weather_data_source`.
- The website version renders only the Monterey-style main weather window: translucent panel, saved-location sidebar, current temperature, metric cards, hourly strip, and 7-day forecast rows. Current wind is shown as a compass-style heading ring with a thin arrow whose size scales with wind speed and a numeric degree heading; clicking it opens the embedded wind detail. The metric cards include precipitation in mm/hr instead of wind, but that card switches to current UV index when precipitation is zero. Clicking a metric card, hourly forecast item, or 7-day row opens one contextual embedded detail panel with selected states, lightweight charts, and compact day/hour breakdowns. The three-dot options menu selects the weather data source: Auto Best, Open-Meteo, MET Norway, or NOAA/NWS for US locations, with Open-Meteo fallback when a selected source is unavailable. The macOS menu bar widget is only in `desktop/big-tuna-weather/`.
