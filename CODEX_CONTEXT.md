...
Brightspace discovery is browser-bot based, not Brightspace API based. `BRIGHTSPACE_COURSE_URLS` is the preferred comma- or newline-separated list of exact classes to inspect. When it is absent, `BRIGHTSPACE_URL` can point at the Brightspace landing/home page and the bot discovers class/course links. The bot reuses a persistent browser profile and reports `login-required` when its saved session expires; automated username/password login is disabled unless `BRIGHTSPACE_AUTOMATED_LOGIN=1`.
