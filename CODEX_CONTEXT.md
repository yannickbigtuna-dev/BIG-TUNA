...
`POST /api/trivia/generate` is authenticated and powers the Lumina Trivia game. Body remains `{ topic?: string, count?: number, model?: string, difficulty?: 'easy'|'medium'|'hard', exclude?: string[] }`. `topic` is trimmed/clamped to 100 chars; `count` is clamped to 1–10 (default 5); `difficulty` pins the returned questions (anything else => mixed); `exclude` now keeps the 80 most-recent question texts (each clamped to 140 chars) for immediate bank/fallback dedupe. The route is intentionally latency-independent from Ollama: it first consumes an in-memory AI bank keyed by topic+difficulty+requested-model, then fills any gap from a validated built-in safety bank (`lib/trivia-generator.js`, 216 questions including generated capital variants), returns immediately, and starts at most one background Ollama refill for that key. The AI refill uses a compact `q/a/c/g/d/e` JSON schema, JSON Schema structured output, `num_ctx:2048`, bounded prediction, `keep_alive:"30m"`, and `pickTriviaModel()` (currently preferring `phi4-mini:latest`; `qwen2.5:1.5b` is installed as a smaller fallback). Malformed/placeholder answers are rejected, all accepted questions are answer-shuffled, and the safety bank honors difficulty plus topic matching when it has relevant candidates. Response metadata now includes `source: 'built-in'|'ai-bank'|'hybrid'`, `aiAvailable`, `refillPending`, and `topicMatched`; valid questions still use `{ id, question, answers:[4], correct, category, difficulty, explanation }`. Ollama failure no longer makes a playable request fail: only exhaustion of both ready and built-in banks returns 503.
...
- Questions are supplied through `POST /api/trivia/generate`: fresh questions come from local Ollama background refills, while a built-in validated safety bank guarantees immediate play during cold starts, model updates, or outages. There is an optional **Topic** text box (blank = random mix) and a **Difficulty** selector (Mixed / Easy / Medium / Hard).
...
- **Batched ready-ahead generation:** the home screen requests `PREBANK`=4 questions in one batch. Starting a run shares any in-flight warm-up instead of launching a competing request and enforces a 4.5-second client deadline (the normal local API response is tens of milliseconds). During play, `ensureBuffer()` requests every missing slot in one batch and keeps `PREFETCH`=3 ready ahead; it is called on render and immediately after an answer is recorded, so the next question should already exist before the player presses Next. Bank/run state is keyed by topic **and** difficulty.
- **Bank warm-up (`warmHomeBank`):** the home-screen bank (current Topic text + Difficulty) is (re-)warmed on load (in parallel with the `/api/eco-ai/status` check, not after it), on Difficulty change, ~450ms after the player pauses typing a custom Topic (`initTopicWarm`, min 3 chars), and whenever the player returns to the home screen (quit/home/tab-to-Play) so a bank drained by the previous run refills for the next one.
...
- Per-user stats/history persist via `Auth.saveSettings('trivia', …)` (best score, games, totals, last 25 runs incl. difficulty) with a `localStorage` fallback; the in-app Rankings tab ranks the user's own completed runs by points. If Ollama is offline the home screen shows a notice, but the built-in safety bank keeps games playable while background AI refills wait for service recovery.
