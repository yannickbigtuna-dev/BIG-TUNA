...
`POST /api/trivia/generate` is authenticated and powers the Lumina Trivia game. Body is `{ topic?: string, count?: number, model?: string, difficulty?: 'easy'|'medium'|'hard', exclude?: string[] }`. `topic` is trimmed/clamped to 100 chars (blank => a random mix of general-knowledge categories); `count` is clamped to 1–10 (default 5) — the client requests **one at a time**; `difficulty` pins the whole batch's difficulty (anything else => a natural mix); `exclude` is the list of already-asked questions (capped at `TRIVIA_MAX_EXCLUDE` = 18 most-recent, each clamped to `TRIVIA_EXCLUDE_ITEM_MAX_LEN` = 140 chars — kept short because it is re-sent in full on every single-question request, so a long list would slow down every generation as a run goes on) that the prompt tells the model not to repeat, so the stateless model avoids duplicates. It reuses the Eco AI Ollama helpers (`getEcoAiStatus`, `ollamaFetchJson`) to call the local model non-streaming with `format:"json"`, a high temperature, a random `seed` for variety, `keep_alive:"30m"` (so the model stays loaded between runs), and `num_predict` capped to roughly `220 + count*260` tokens (bounds worst-case generation latency for the small JSON payload expected). The model output is parsed and passed through `validateTriviaQuestions()`, whose dedupe set is **seeded from `exclude`** (normalised via `normalizeQuestionText`); it drops any item that is not exactly four distinct answers with one valid correct index (accepting either a numeric `correct` index or a `correct`/`answer` answer-text fallback), then shuffles each question's answers and recomputes the correct index. Each question also carries a one-sentence `explanation` (memory aid). Returns `{ ok, model, topic, difficulty, questions:[{ id, question, answers:[4], correct, category, difficulty, explanation }] }`, `503` when Ollama is offline, or `502` when no usable questions were produced. It does not persist anything server-side.
...
- All questions are AI-generated by the local Ollama model through `POST /api/trivia/generate`. There is an optional **Topic** text box (blank = random mix) and a **Difficulty** selector (Mixed / Easy / Medium / Hard).
...
- **One-at-a-time generation:** the app shows the first question as soon as it arrives, then prepares each following question in the background (`ensureBuffer`, one request at a time, keeping `PREFETCH`=2 ready ahead) while the player answers. A few questions are pre-generated on load (`prebank`) for an instant first game. Bank/run state is keyed by topic **and** difficulty.
...
- **No duplicates:** the client remembers every question served (`run.seen`, normalised) and sends the recent set (capped, see `EXCLUDE_MAX` above — must match server `TRIVIA_MAX_EXCLUDE`) as `exclude` on each request; the single `addUnique()` choke-point drops any repeat before it can be shown, and the server dedupes again against the same `exclude` list. This is the intended architecture for "remembering" asked questions.
- Per-user stats/history persist via `Auth.saveSettings('trivia', …)` (best score, games, totals, last 25 runs incl. difficulty) with a `localStorage` fallback; the in-app Rankings tab ranks the user's own completed runs by points. If Ollama is offline the home screen shows an offline notice and generation returns 503.
