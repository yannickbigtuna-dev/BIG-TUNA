...
function projectYml(spec) {
  ...
  const dependencies = [];
  if (target.homeScreenWidget) dependencies.push('      - target: AppWidget');
  ...
  if (target.homeScreenWidget) sections.push(
    '  AppWidget:', '    type: app-extension', '    platform: iOS', `    deploymentTarget: ${quote(spec.app.minimumIOS)}`, '    sources:', '      - path: Sources/Widget', '      - path: Sources/Shared', '      - path: Generated', ...(target.liveActivities ? ['      - path: Sources/LiveActivity'] : []), '    entitlements:', '      path: Generated/Widget.entitlements', '    settings:', '      base:', `        PRODUCT_BUNDLE_IDENTIFIER: ${ids.widget}`, `        PRODUCT_NAME: ${quote(`${spec.app.name} Widgets`)}`, '        GENERATE_INFOPLIST_FILE: YES', '        SKIP_INSTALL: YES', '        INFOPLIST_KEY_NSExtension: {NSExtensionPointIdentifier: com.apple.widgetkit-extension}'
  );
  ...
  if (target.watchWidgets) sections.push(
    '  AppWatchWidget:', '    type: app-extension', '    platform: watchOS', `    deploymentTarget: ${quote(spec.app.minimumWatchOS)}`, '    sources:', '      - path: Sources/WatchWidget', '      - path: Sources/Shared', '      - path: Generated', '    entitlements:', '      path: Generated/WatchWidget.entitlements', '    settings:', '      base:', `        PRODUCT_BUNDLE_IDENTIFIER: ${ids.watchWidget}`, `        PRODUCT_NAME: ${quote(`${spec.app.name} Watch Widgets`)}`, '        GENERATE_INFOPLIST_FILE: YES', '        SKIP_INSTALL: YES', '        INFOPLIST_KEY_NSExtension: {NSExtensionPointIdentifier: com.apple.widgetkit-extension}'
  );
...
try {
  ...
  mkdirp(output); copyTree(template, output);
  ...
  if (fs.existsSync(widgetFile)) {
    ...
    fs.writeFileSync(widgetFile, source.replace('// LIVE_ACTIVITY_WIDGET', spec.targets.liveActivities ? 'FactoryLiveActivity()' : '').replace('// LOCK_SCREEN_FAMILIES', spec.targets.lockScreenWidget ? '.accessoryCircular, .accessoryRectangular, .accessoryInline' : ''));
  }
  ...
  writeJson(path.join(output, '.factory-targets.json'), { schemaVersion: 1, app: spec.app, factory: spec.factory, bundleIds: bundleIds(spec), targets: spec.targets, capabilities: spec.capabilities });
